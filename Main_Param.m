...
%             save('para_hex_matlab_vbg.mat');
            if is_het_bg > 0
                if is_nb_10 > 0
                    save('para_hex_nb10_het.mat');
                else
                    save('para_hex_matlab_vbg.mat');
                end
            else
                if is_nb_10 > 0
                    save('para_hex_nb10_homo.mat');
                else
                    save('para_hex_matlab.mat');
                end
            end
    ...
    if is_het_bg > 0
        if is_nb_10 == 0
            ...
            load('para_hex_nb10_het.mat');
        end
    else
        if is_nb_10 == 0
            load('para_hex_matlab.mat');
        else
            load('para_hex_nb10_homo.mat');
        end
    end
end
...
%     base_bg_rate = 500; %% 4 = 25% = 500M/2G
    if is_het_bg > 0
        if is_nb_10 > 0
            bg_filename = strcat('/export/home/Yudi_Huang/ns-3-dev/scratch/CrossPathParam/bg_ratio_nb10_',string(iter),'.csv');
        else
            bg_filename = strcat('/export/home/Yudi_Huang/ns-3-dev/scratch/CrossPathParam/bg_ratio_',string(iter),'.csv');
        end
    else
        bg_filename = '/export/home/Yudi_Huang/ns-3-dev/scratch/CrossPathParam/bg_ratio_0.csv';
    end
    para_probe = py.dict(pyargs('E_cur_idxlist',E_cur_idxlist,'e_new_idx',1,'n_calibrate_pkt',1000, 'n_uePerGnb', n_uePerGnb, 'n_probes', 5000, 'n_UE', n_UE, 'is_w_probing', 0, 'bg_filename', bg_filename, 'task_name', 'CrossPathParam'));
    py.XPAttack_utils.init_setup(para_probe);
    if is_het_bg > 0
        back_ground_traffic = ceil(base_bg_ratio ./ cell_background_percent{iter} * base_bg_rate);
    else
        back_ground_traffic = back_ground_traffic * ones(n_iab, n_iab);
    end
...
%             probe_param_interval = probe_pkt_size * 8 / (tau_path_bw); % microseconds
            for k = 0 : n_BW_slice-1
                ...
                para_probe = py.dict(pyargs('E_cur_idxlist', E_to_tbs, 'shape', pareto_shape, 'tb', [tb-n_iab], 'probe_param_interval', probe_param_interval, 'probe_pkt_size', probe_pkt_size, 'tau', vec_tau(m)-n_iab, 'task_name', 'CrossPathParam'));
                %% Prepare Python Env
                if count(py.sys.path,'') == 0
                    insert(py.sys.path,int32(0),'');
                end
...
% %             save('delays_log_nb10_het_n5k.mat','delays_log', 'delays_log_burst');
        end
...
% load('delays_log_het_2k_backup.mat');
% load('delays_log_het1K.mat');
% load('delays_log_het200.mat');
load('delays_log_nb10_homo_2k.mat');
...
cell_param_gg1_mu_exp = cell(1, n_MC);
cell_param_gg1_lambda_exp = cell(1, n_MC);
...
%     param_est = param_MM1_empirical(delays_log(sum(vec_n_shared_links(1:iter-1))+1, sum(vec_n_shared_links(1:iter)), :), param_est, tbs, ABW_true, sp, T_true, tick_bw);
    ...
    [mu_est_gg1, lambda_est_gg1] = param_GG1_empirical(delays_delta(idx_start: idx_end, :), tbs, ABW_true, sp, tick_bw); %% for one iter in MC
    mask = param_est > 0;
    if is_delta > 0
        ...
        cell_param_gg1_mu_exp{iter} = mu_est_gg1(mask) - bw_base;
        cell_param_gg1_lambda_exp{iter} = lambda_est_gg1(mask) - (bw_base - ABW_true(mask));
    else
        cell_param_mm1_exp{iter} = param_est;
        cell_param_md1_mu_exp{iter} = mu_est;
        cell_param_md1_lambda_exp{iter} = lambda_est;
        cell_param_gg1_mu_exp{iter} = mu_est_gg1;
        cell_param_gg1_lambda_exp{iter} = lambda_est_gg1;
    end
end
filename = strcat( "res_param", str_delta, str_het, str_nb, '_2k', '.mat' );
save(filename, 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');

% save('res_param_delta_1w.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
% save('res_param_delta_het_2k.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
% save('res_param_delta_het_1k.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
% save('res_param_delta_het_200.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
% 
% save('res_param_1w.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
% save('res_param_het_2k.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
% save('res_param_het_1k.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
% save('res_param_het_200.mat', 'cell_param_mm1_exp', 'cell_param_md1_mu_exp', 'cell_param_md1_lambda_exp', 'cell_param_gg1_mu_exp', 'cell_param_gg1_lambda_exp');
