...
if is_recompute == 1
    ...
    for i = 1: n_MC
        ...
        if is_het_bg > 0
            ...
            if is_nb_10 > 0
                filename = strcat('bg_ratio_nb10_',string(i),'.csv');
            else
...
if do_w_probing
    ...
    list_n_probes = [100, 5000, 1000, 5000];
%     list_n_probes = [2000];
    acc = zeros(size(list_n_probes, 2), n_MC);
    BT_set = find(T_true(root,:));
    BT_set = BT_set( BT_set <= n_iab );
    pareto_shape = 2.04;
%     cell_miss = cell(size(list_n_probes, 2), n_MC);
%     cell_fa = cell(size(list_n_probes, 2), n_MC);
...
%         py.XPAttack_utils.init_setup(para_probe);
        for iter = 1 : n_MC
            if is_het_bg > 0
                if is_nb_10 > 0
                    bg_filename = strcat('/export/home/Yudi_Huang/ns-3-dev/scratch/Category_inference/bg_ratio_nb10_',string(iter),'.csv');
                else
                    bg_filename = strcat('/export/home/Yudi_Huang/ns-3-dev/scratch/Category_inference/bg_ratio_',string(iter),'.csv');
                end
            else
                bg_filename = '/export/home/Yudi_Huang/ns-3-dev/scratch/Category_inference/bg_ratio_0.csv';
            end
            ...
            py.XPAttack_utils.init_setup(para_probe);
            tb = cell_tb{iter};
%             W_est_cell_each_iter = cell(1, length(tb));
...
%                 idx_tb = 2;
            ...
            [acc(k, iter), list_miss, list_fa] = inference_acc(W_true_cell{iter},W);
            W_est_cell{iter} = W;
            save(strcat('W_acc_', string(k), '_' , string(iter) ,'.mat'), 'W', 'acc', 'W_est_cell', 'list_miss', 'list_fa');
        end
    end
