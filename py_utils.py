...
def create_overlay_nodes(n_overlay_nodes, spr_table, n_nodes):
...
'''Write Functions'''
def convert_route_to_file(res_x, tunnel_list, spr_table):
    # tunnels * demands
...
def write_tunnel_demands(tunnel_demands:list, tunnel_list:list):
    # Write into files
...
def wr_overlay_nodes(loc_overlay_nodes:list):
    file_name = "overlay.txt"
    str_wr = str(len(loc_overlay_nodes)) + ': ' + ' '.join(str(idx) for idx in loc_overlay_nodes)
    with open(file_name, "w") as f:
        f.write( str_wr )
        
def write_setup(graph_file_name):
    with open("setup.txt", "w") as stf:
        stf.write(graph_file_name)
